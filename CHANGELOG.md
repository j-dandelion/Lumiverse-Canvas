...
- **Active secondary tab icon was indistinguishable from inactive in some themes.** `showSecondaryTab` (and the `mouseleave` handler in `addSecondaryTabButton`) wrote the active button's `color`, label `color`, and `boxShadow` inset indicator as `var(--lumiverse-primary)` with no fallback. When the CSS variable was undefined, absent on `documentElement` at computation time, or resolved to a non-purple value (e.g. `pL=75` in the engine dark-mode formula produces near-white — `--lumiverse-primary` matches `--lumiverse-text-muted` in that case), the active tab icon and label rendered the same white-65% as the inactive state, making it impossible to tell which tab was active. Fix: added `, #9370db` (matches `~/Lumiverse/frontend/src/theme/variables.css:3` default) as the literal fallback to all 6 `var(--lumiverse-primary)` references in `src/tabs/buttons.ts` (the active-state color, active-state label color, and `boxShadow` inset indicator — 2 locations per state from the `mouseleave` and the `showSecondaryTab` paths). Files: `src/tabs/buttons.ts`. Covered by 17 tests in `src/tabs/__tests__/buttons.test.ts` (T10 active/inactive color, T11 switch active tab, T12 idempotent re-call).
